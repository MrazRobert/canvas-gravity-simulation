...
gui.add(simulation, 'gravity', 0, 5, 0.01).onChange(function () {
  return setRadius();
});
gui.add(simulation, 'friction', 0, 1.1, 0.01).onChange(function () {
  return setRadius();
});
var ballProperties = gui.addFolder('ball properties');
ballProperties.add(simulation.balls, 'count', 0, 600, 1).onChange(function () {
  return ballAdderAndRemover();
});
ballProperties.add(simulation.balls, 'minRadius', 4, 30, 1).onChange(function () {
  return setRadius();
});
ballProperties.add(simulation.balls, 'maxRadius', 4, 30, 1).onChange(function () {
  return setRadius();
});
...
var colors = ['#FA0C8F', '#FADE19', '#AE01FA', '#21FA19', '#310CFA']; // Event Listeners
...
function init() {
  ...
  for (var i = 0; i < simulation.balls.count; i++) {
...
function createBall() {
  var _simulation$balls = simulation.balls,
      minRadius = _simulation$balls.minRadius,
      maxRadius = _simulation$balls.maxRadius;
  var radius = _utils__WEBPACK_IMPORTED_MODULE_0___default().randomIntFromRange(minRadius, maxRadius);
...
function ballAdderAndRemover() {
  ...
  var ballsCount = Math.abs(ballsLength - simulation.balls.count);
...
function setRadius() {
  var _simulation$balls2 = simulation.balls,
      minRadius = _simulation$balls2.minRadius,
      maxRadius = _simulation$balls2.maxRadius;
...
function animate() {
  ...
  c.fillStyle = "rgba(0, 0, 0, ".concat(simulation.alpha, ")");
